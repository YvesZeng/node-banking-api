...
router.post('/', checkSchema(newBankAccountSchema), async (req: Request, res: Response) => {
    ...
    const bankAccountData = { customerId: req.body.customerId, depositAmount: req.body.depositAmount };
    try {
        const bankAccount = await createBankAccount.execute(bankAccountData);
        return res.status(StatusCodes.CREATED).json(bankAccount);
    } catch (err) {
        if (err instanceof CustomerNotFound) return res.status(StatusCodes.BAD_REQUEST).json({ message: err.message });
    }
    return res;
});
...
router.get('/:bankAccountId', (req: Request, res: Response) => {
    ...
    if (!parseInt(bankAccountId)) return res.status(StatusCodes.BAD_REQUEST).json({ message: 'Bank account id needs to be an integer' });
    const retrieveBankAccountBalance = container.get<RetrieveBankAccountBalance>(TYPES.RetrieveBankAccountBalance);
    retrieveBankAccountBalance.execute(parseInt(bankAccountId))
        .then((balance) => res.status(StatusCodes.OK).json({ balance }))
        .catch((err) => {
...
router.get('/:bankAccountId/transfers', (req: Request, res: Response) => {
    ...
    listTransferHistory.execute(parseInt(bankAccountId)).then((transfers) => res.status(StatusCodes.OK).json(transfers));
});

router.get('/:bankAccountId/loans', async (req: Request, res: Response) => {
    const { bankAccountId } = req.params;
    if (!parseInt(bankAccountId)) return res.status(StatusCodes.BAD_REQUEST).json({ message: 'Bank account id needs to be an integer' });
    const loanRepository = container.get<LoanRepository>(TYPES.LoanRepository);
    try {
        const loans = await loanRepository.findByAccountId(parseInt(bankAccountId));
